...
import { createRoutes, rateLimit, requireToken } from './routes.js';
...
async function main() {
  ...
  if (!process.env.API_TOKEN && process.env.NODE_ENV === 'production') {
    console.warn('[Auth] API_TOKEN not set — mutating endpoints are unprotected');
  }
  ...
  app.post('/api/test-notification', requireToken, async (req, res) => {
  ...
  // Unknown API routes get JSON 404, not the SPA shell
  app.use('/api', (req, res) => {
